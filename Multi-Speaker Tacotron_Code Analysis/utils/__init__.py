...
class ValueWindow():
    def __init__(self, window_size=100):  # 기본 window 크기 100
    ...
    @property
    def sum(self):
        return sum(self._values)  # 리스트 내의 모든 원소의 합 리턴

    @property
    def count(self):
        return len(self._values)  # 리스트 길이 리턴

    @property
    def average(self):
        return self.sum / max(1, self.count)  # 원소 값 평균 리턴

    def reset(self):
        self._values = []  # _values 리스트 초기화
